pub mod template {
    ...
    use crate::dictionary::{dictionary::SearchPattern, word::word::{get_word_tags, get_wrapper_content}};
        ...
        if template_wrapped.is_some() {
            ...
            for section in template_sections {
                ...
                if format_template.is_match(section_text) {
                    output.push(TemplateElement { text: None, template: Some(parse_template_term(section_text))});
                } else {
                    output.push(TemplateElement { text: Some(section_text.to_string()), template: None});
                }
    ...
    fn test_parse_template() {
        assert!(parse_template("TEMPLATE([[Metal, Wood]] [[Mammal]] Tavern), TAG(Institution), TAG(Restaurant)").unwrap().template.len().eq(&4));
